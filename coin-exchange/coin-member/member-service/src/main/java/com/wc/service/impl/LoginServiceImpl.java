...
import com.wc.geetest.GeetestLibResult;
import com.wc.model.LoginForm;
import com.wc.model.LoginUser;
import com.wc.service.LoginService;
import com.wc.util.IpUtil;
...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginUser login(LoginForm loginForm) {
        ...
        if(tokenResponseEntity.getStatusCode() == HttpStatus.OK){
            ...
            loginUser = new LoginUser(loginForm.getUsername(),jwtToken.getAccessToken(),jwtToken.getExpiresIn(),jwtToken.getRefreshToken());
    ...
    private void checkFormData(LoginForm loginForm) {
        ...
        if(status == 1){
            /*自定义参数,可选择添加user_id客户端用户的唯一标识，确定用户的唯一性；作用于提供进阶数据分析服务，可在register和validate
            . 接口传入，不传入也不影响验证服务的使用；若担心用户信息风险，可作预处理(如哈希处理)再提供到极验client_type客户端类型，web：电脑上的浏览器；h5
            ：手机上的浏览器，包括移动应用内完全内置的web_view；native：通过原生sdk植入app应用的方式；unknown：未知ip_address客户端请求sdk服务器的ip地址
            */
