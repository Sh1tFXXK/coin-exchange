...
import com.wc.vo.UseAuthInfoVo;
...
public class UserController implements UserServiceFeign {
    ...
    public R<Page<User>> findByPage(@ApiIgnore Page<User> page ,
        ...
        Page<User>userPage=
                userService.findByPage(page,mobile,userId,userName,realName,status,null) ;
    ...
    public R<UseAuthInfoVo> getUseAuthInfo(Long id){
        ...
        if (user != null){
            ...
            if (reviewsStatus ==null || reviewsStatus == 0 ){
                ...
                userAuthInfoList = userAuthInfoService.getUserAuthInfoById(id);
            }else {
    ...
    public R updateUserAuthStatus(@RequestParam()Long id, @RequestParam() Byte authStatus, @RequestParam()Long authCode, String remark) {
        //审核: 1 修改user 里面的reviewStatus
        // 2在authAuditRecord里面添加一条记录
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name="updatePhoneParam",value="updatePhoneParam的 json数据")
    })
